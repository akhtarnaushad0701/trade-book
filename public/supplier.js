...
console.log("Loading customer", id, act);
  ...
  try {
    ...
    if (!res.ok) {
      setMessage('Supplier not found', true);
      return;
    }
    const data = await res.json();
    document.getElementById('supID').value = data.supID;
    document.getElementById('supName').value = data.supName || '';
    document.getElementById('supContact').value = data.supContact || '';
    document.getElementById('supAdd').value = data.supAdd || '';
    document.getElementById('supRemark').value =data.supRemark || '';
   

    if (act === '1') {
      currentAct = '1';
      document.getElementById('act').value = '1';
      document.getElementById('submitBtn').textContent = 'âœ” Update';
     document.getElementById('submitBtn').value='✔ Update';
	  ...
	  document.getElementById('submitBtn').textContent = 'âœ” Delete';
...
function validateForm() {
  ...
  if (!name) {
   document.getElementById('supName').style.border = "2px red solid";
    ok = false;
  }
  return ok;
}
...
form.addEventListener('submit', async (e) => {
  ...
  const sup = {
    ...
    supContact: document.getElementById('supContact').value,
    supAdd: document.getElementById('supAdd').value,
    supRemark: document.getElementById('supRemark').value,
  };
...
function setBlank()
{
	...
	document.getElementById('supAdd').value ="";
	document.getElementById('supRemark').value ="";
